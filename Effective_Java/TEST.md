...
- [項目81 wait と notify よりも並行処理ユーティリティを選ぶ](#項目81-wait-と-notify-よりも並行処理ユーティリティを選ぶ)
  - [解答](#解答-33)
- [(省略) 項目82 スレッドの安全性を文書化する](#省略-項目82-スレッドの安全性を文書化する)
...
スレッドは(1)と(2)が同じであるため、極力使用するのは避ける。代わりに(3)と(4)を使用する。それぞれ(1)と(3)、(2)と(4)が対応しており、(3)の具体的なクラスには(5)と(6)が存在する。
...
「wait と notify よりも並行処理ユーティリティを選ぶ」について述べよ

## 解答
Object.wait/notify を使用して並列処理を実現させるよりも、java.util.concurrent のユーティリティを使用して実現させる方が安全である。

# (省略) 項目82 スレッドの安全性を文書化する
...
以下の文章は Object.readObject の実装に関する文章である。
1. private フィールドのセットに関しては(1)を使用する  
2. 不変式を使用してオブジェクトの状態をチェックし、不正な状態であれば `(2)`をスローする。このチェックは(1)後に行う  
3. デシリアライズの前に呼び出すとプログラムが失敗するので、クラス内の(3)なメソッドは呼び出してはいけない。

## 解答
1. 防御的コピー  
2. InvalidObjectException  
3. オーバーライド可能

# 項目89 インスタンス制御に関しては、readResolve よりも enum型を選ぶ

シリアライズ可能なシングルトンインスタンスを実装する場合にはクラスではなく enum で実装するのが望ましいとされている。その理由を述べよ。

## 解答
シリアライズ可能なシングルトンクラスはデシリアライズによってシングルトンの制約が破られる危険性があるが、enumによるシングルトンはJVMがそのインスタンスがただ一つ存在することを保証するので安全であるため。

# 項目90 シリアライズされたインスタンスの代わりに、シリアライズ・プロキシを検討する
シリアライズプロキシとは何か述べよ。

## 解答
シリアライズプロキシとは、内部にフィールド構成がエンクロージングインスタンスと同じである private static なクラスを作成し、それをシリアライズ、デシリアライズの対象とするパターンのことを指す。