...
Effective Java の項目49 「パラメータの正当性を検査する」について述べよ
...
# 項目79 過剰な同期は避ける
過剰な同期は意図しない動作を生じさせる原因となる。例えば、同期処理の中でロックした資源に対して何か変更を加えるような処理を実装した場合、 `ConcurrentModificationException`が実行時にスローされる。これを避けるためには、ロックしようとする資源の(1)を取り、その(1)に対して処理を行うようにすれば解消できる。資源が List である場合は、(2)を使用すると(1)を取る操作を内部で行ってくれるためコードはシンプルになる。
