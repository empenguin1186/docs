...
- [項目80 スレッドよりもエグゼキュータ、タスク、ストリームを選ぶ](#項目80-スレッドよりもエグゼキュータタスクストリームを選ぶ)
  - [参考文献](#参考文献)
- [項目81 wait と notify よりも並行処理ユーティリティを選ぶ](#項目81-wait-と-notify-よりも並行処理ユーティリティを選ぶ)
- [項目82 スレッドの安全性を文書化する](#項目82-スレッドの安全性を文書化する)
...
- [項目85 Java のシリアライズよりも代替手段を選ぶ](#項目85-java-のシリアライズよりも代替手段を選ぶ)
  - [問題](#問題-28)
- [項目86 Serializable を細心の注意を払って実装する](#項目86-serializable-を細心の注意を払って実装する)
- [項目87 カスタムシリアライズ形式の使用を検討する](#項目87-カスタムシリアライズ形式の使用を検討する)
- [項目88 防御的に readObject メソッドを書く](#項目88-防御的に-readobject-メソッドを書く)
- [項目89 インスタンス制御に関しては、readResolve よりも enum型を選ぶ](#項目89-インスタンス制御に関してはreadresolve-よりも-enum型を選ぶ)
- [項目90 シリアライズされたインスタンスの代わりに、シリアライズ・プロキシを検討する](#項目90-シリアライズされたインスタンスの代わりにシリアライズ・プロキシを検討する)
...
1. public あるいは protected のコンストラクタが実装されていないクラスのサブクラスを作成することができない。  
...
```
このように、フィールドに`DoubleBinaryOperator`という関数型�������ンターフェースを追加し、それに`apply`メソッドの処理を移譲させるという構成にしている。ちなみに、enum で定義した処理は、インスタンスフィールドすることができないので注意。  
...
# 項目67 注意して最適化する
- 早いプログラムよりも優れたプログラムを書くことを意識する。優れたプログラムとは情報隠蔽が適切であるプログラムのことであり、モジュール間は疎結合で修正による影響を最小限に留めることができるものを指す。
- 設計の段階でパフォーマンスを考慮する。例えば外部APIをコールする際の通信プロトコルや永続化の方法などは後から変更することは難しいので、設計段階で十分考慮する。
- 実装が完了した段階でパフォーマンステストを行い、要件を満たしているのであればそのままでいいし満たしていないのであれば修正を行う。

# 項目68 一般的に受け入れられている命名規約を守る
- 保守性を考慮して、命名規則は一般的なものに従う

# 項目69 例外的状態にだけ例外を使う
- 例外は例外的状態に対して使われるように実装されているため、正常系の処理において例外におけるハンドリングは避けた方が良い。

# 項目70 回復可能な状態にはチェックされる例外を、プログラミングエラーには実行時例外を使う
- Java においてスローされる例外には以下の３つが存在する
1. チェックされる例外  
2. 実行時例外  
3. エラー  
- APIの設計者は、クライアントが例外がスローされた場合にその状態から正常の状態に回復できる可能性が存在する場合にのみチェックされる例外をスローする。例えば、預金口座からの引き出しで預金金額を超えた金額を出金しようとした場合にはチェックされる例外をスローする。それに伴いその例外に関連したメソッドも実装しておく例えば預金口座の残高を取得するメソッドなどがそれに当たる。
...
# 項目79 過剰な同期は避ける
- 過剰な同期は意図しない動作を生じさせる原因となる。例えば、同期処理の中でロックした資源に対して何か変更を加えるような処理を実装した場合、 `ConcurrentModificationException`が実行時にスローされる。これを避けるためには、ロックしようとする資源のスナップショットを取り、そのスナップショットに対して処理を行うようにすれば解消できる。資源が List である場合は、`CopyOnWriteArrayList`を使用するとスナップショットを取る操作を内部で行ってくれるためコードはシンプルになる。
- 同期処理を過剰に行うことを避ける。その処理が同期処理である必要があるかどうかは検討し、必要である場合は同期の範囲を少なくする。

# 項目80 スレッドよりもエグゼキュータ、タスク、ストリームを選ぶ
- スレッドは処理の単位と実行の機構が同じであるため、極力使用するのは避ける。代わりにエグゼキュータサービスとタスクを使用する。それぞれ処理の単位をタスク、実行の機構をエグゼキュータサービスと呼ぶ。

シングルスレッドで機能するエグゼキュータサービスを定義するには、以下のようなコードを記述する。
```java
ExecutorService exec = Executors.newSingleThreadExecutor();
```

タスクは Runnable, または Callable クラスで定義され、エグゼキュータサービスに渡すことで実行される。
```java
// 実行
...
exec.shutdown();
```
エグゼキュータサービスはスレッドプールの機能を有している。あらかじめスレッドを作成しておき、タスクが投げられた場合に存在するスレッドにそのタスクを割り当てる。またスレッドプールの設定によって異なるファクトリメソッドを呼ぶ必要があり、例として`Executors.newCachedThreadPool()`や`Executors.newFixedThreadPool(int n)`が存在する。`Executors.newCachedThreadPool()`は使用できるスレッドが存在しなければ新しくスレッドを作成するもので、`Executors.newFixedThreadPool(int n)`はあらかじめ決めた数だけのスレッドを作成するものである。

## 参考文献
https://java.keicode.com/lang/multithreading-executor.php

# 項目81 wait と notify よりも並行処理ユーティリティを選ぶ
- Object.wait() はこのメソッドを呼び出したスレッドの処理を停止させるメソッドである。Object.notify() は wait() で停止したスレッドを再開させるメソッドである。これらを組み合わせて独自で並列処理を実装するよりも、java.util.concurrent ユーティリティを使用して実装した方が安全である。

# 項目82 スレッドの安全性を文書化する
- クラスのメソッドが並列で使用される場合、スレッドの安全性の有無については文書化するべきである。
...
- クラスをシリアライズ可能にすることは以下のコストが伴う  
1. 一旦そのクラスをリリースすると、基本的にはその形式をサポートし続けなければならず、変更しづらくなる。  
2. シリアライズは基本的には言語外の仕組みであるため、外部からの不正なデータを内部に取り込むなどの、バグやセキュリティホールの可能性を増大させる。  
3. 新しいバージョンがリリースされた場合に、新旧のバージョンが互換性があるかどうかを検証しなければならず、テストケースが増える。  

# 項目87 カスタムシリアライズ形式の使用を検討する
- オブジェクトのシリアライズ形式は真剣に検討する。定義したAPIが後のバージョンで削除できないことと同じように、シリアライズ形式から既存のフィールドを削除することはできないからである。

# 項目88 防御的に readObject メソッドを書く
- readObject メソッドを書くときのガイドラインは以下。  
1. private フィールドのセットに関しては防御的コピーを使用する  
2. 不変式を使用してオブジェクトの状態をチェックし、不正な状態であれば `InvalidObjectException`をスローする。このチェックは防御的コピーの後に行う  
3. デシリアライズの前に呼び出すとプログラムが失敗するので、クラス内のオーバーライド可能なメソッドは呼び出してはいけない。

# 項目89 インスタンス制御に関しては、readResolve よりも enum型を選ぶ

- シングルトンクラスをシリアライズ可能にする場合は極力 enum によるインスタンス制御を用いて実現させる。シリアライズ可能なシングルトンクラスはデシリアライズによってシングルトンの制約が破られる危険性があるが、enumによるシングルトンはJVMがそのインスタンスがただ一つ存在することを保証するので安全である。  
```java
public enum Elvis {
    INSTANCE;
...
- シリアライズプロキシとは、内部にフィールド構成がエンクロージングインスタンスと同じである private static なクラスを作成し、それをシリアライズ、デシリアライズの対象とするパターンのことを指す。これにより攻撃者はエンクロージングクラスを用いて不正な状態のインスタンスをデシリアライズさせようとしても、シリアライズ、デシリアライズは外部に公開されていない内部クラスが対象なので不可能となる。